...
// attachLadderSubRenditions stores the backend ladder's rendition list,
// unless the frontend registered its own list while the ladder was in
// flight — the player's POST /api/media/subs/ payload wins, so the ladder
// finishing late (possibly with zero results) cannot drop the user-visible
// tracks. Reports whether the ladder list was applied.
func (r *Resolver) attachLadderSubRenditions(token string, subs []SubRendition) bool {
	if len(subs) > maxSubRenditions {
		subs = subs[:maxSubRenditions]
	}
	r.mu.Lock()
	...
	s, ok := r.sessions[token]
	if !ok || time.Now().After(s.expiresAt) {
		return false // session gone while the ladder ran
	}
	// Only transition Pending → Done: a state of subsDone means the frontend
	// already called SetSubRenditions during the ladder fetch.
	if !s.subsState.CompareAndSwap(subsPending, subsDone) {
		return false
	}
	s.subs = subs
	if s.subsDone != nil {
...
func (r *Resolver) maybeAttachSubRenditions(token string, req MediaRequest) {
	...
	go func() {
		...
		if r.attachLadderSubRenditions(token, subs) {
			log.Printf("[MediaResolver] embedded %d subtitle renditions into manifest id=%s provider=%s", len(subs), req.ID, req.Provider)
			return
		}
		// The ladder finished too late: the session expired or the frontend
		// registered its own rendition list in the meantime (which wins).
		log.Printf("[MediaResolver] subtitle ladder skipped (session gone or frontend renditions already registered) id=%s provider=%s", req.ID, req.Provider)
	}()
}
