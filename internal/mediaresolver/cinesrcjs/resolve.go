...
type assets struct {
	...
	donut   []byte
	powWasm []byte
	actions map[string]string // server-action ids discovered from the app chunk, by name
	fetched time.Time
}
...
var (
	...
	// hardcoded (redeployed ids once made every getStream call return a
	// 200 response with no cipher at all).
	reActionRef = regexp.MustCompile(`createServerReference\)?\("([A-Za-z0-9]{32,64})"[^()]{0,200}?"(getProviderList|getStream)"\)`)
)
...
func (r *Resolver) Resolve(ctx context.Context, mediaType, tmdbID, season, episode string, servers []string) (*Result, error) {
	// Snapshot the timeout locally: Resolve may run concurrently (the bridge
	// bounds it to two) and writing the exported field unsynchronized would
	...
	// Explicit caller-supplied servers must survive the asset-refresh retry
	// below; only the discovered list is re-fetched.
	...
	for pass := 0; pass < 2; pass++ {
		...
		if !strings.Contains(lastErr.Error(), "no r2 cipher") || !r.resetAssets() {
			break
		}
		// The response shape was fine but the cipher was absent on every
		// server: the app chunk (and with it the action ids) likely rotated
		// mid-cache. Re-fetch the assets once and try the list again.
		r.logf("action ids possibly stale; refreshing assets")
		if err := r.ensureAssets(ctx, origin, ua); err != nil {
			return nil, fmt.Errorf("cinesrcjs: refresh after cipher-less resolve: %w", err)
		}
		if len(originalServers) == 0 {
			servers = r.providerList(ctx, origin, ua)
		} else {
...
func (r *Resolver) attemptServer(ctx context.Context, origin, ua, mediaType, tmdbID, season, episode, pagePath, srv string) (*Result, error) {
	...
	// Snapshot the cached assets under the lock: a concurrent Resolve may
	// resetAssets() (nil) or replace them mid-flight, and the scripts below
	// run from the snapshot bytes.
	r.mu.Lock()
	assets := r.assets
	r.mu.Unlock()
	if assets == nil {
	...
	// The listener script declares `var __d6 = null`, so a registered module
	// turns the global from null into an object; test for null explicitly or
	// a failed registration silently falls through to a gc() timeout.
	d6v := rtD6.vm.GlobalObject().Get("__d6")
	if d6v == nil || goja.IsUndefined(d6v) || goja.IsNull(d6v) {
...
func (r *Resolver) providerList(ctx context.Context, origin, ua string) []string {
	...
	if err == nil {
		...
		if rerr == nil {
			...
			req.Header.Set("next-action", r.providerListActionID())
...
// streamActionID / providerListActionID return the Next.js server-action ids
// discovered from the app chunk, falling back to the last known hardcoded
// values when discovery came up empty.
func (r *Resolver) streamActionID() string {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.assets != nil && r.assets.actions["getStream"] != "" {
		return r.assets.actions["getStream"]
	}
	return getStreamAction
}

func (r *Resolver) providerListActionID() string {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.assets != nil && r.assets.actions["getProviderList"] != "" {
		return r.assets.actions["getProviderList"]
	}
	return getProviderListAction
}

// resetAssets drops the cached assets so the next ensureAssets re-scans the
// site's chunks (and rediscovers the action ids). Reports whether there was
// anything cached to reset.
func (r *Resolver) resetAssets() bool {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.assets == nil {
		return false
...
func (rt *runtime) decrypt(ctx context.Context, cipher string) (*Result, error) {
	...
	for i := 0; i < 800; i++ {
		...
		if out != nil && !goja.IsUndefined(out) && !goja.IsNull(out) {
			...
			src := strings.TrimSpace(dr.URL[0].URL)
			if src == "" {
				return nil, errors.New("dr returned no urls")
			}
			// The embed sometimes returns a root-relative playlist path
			// ("/api/playlist/…") instead of an absolute URL; absolutize it
			// against the challenge origin so downstream validation passes.
			if u, err := url.Parse(src); err == nil && u.Scheme == "" && u.Host == "" {
				if abs, aerr := resolveURL(src, rt.origin); aerr == nil {
					src = abs
				}
			}
			return &Result{
...
func (r *Resolver) ensureAssets(ctx context.Context, origin, ua string) error {
	...
	var (
		...
		scanOne  = func(name string) {
			...
			for _, am := range reActionRef.FindAllStringSubmatch(string(chunk), -1) {
				actions[am[2]] = am[1]
			}
		}
	)
