...
func newPowRuntime(wasmBytes []byte) (*powRuntime, error) {
	...
	return &powRuntime{eng: eng, mod: mod, mem: mem, fnA: fnA, fnB: fnB}, nil
}

// close releases the wazero runtime and module. Calls are serialized with
// solve by the Resolver's mutex at every replacement site.
func (p *powRuntime) close() {
	if p == nil {
		return
	}
	p.mu.Lock()
