...
type vidsrcmeAPIResponse struct {
	...
	// DefaultSubs mirrors the upstream payload shape but is not consumed:
	// subtitles for vidsrcme come from the generic server-side ladder
...
func (r *Resolver) tryVidsrcmeDirect(parent context.Context, req MediaRequest) (string, bool) {
	...
	// Dashboard resolution from the validated manifest.
	r.noteStreamHeightToken(token, vr.MasterText, "")
...
func (r *Resolver) resolveVidsrcmeDirect(ctx context.Context, req MediaRequest) (*directResolution, error) {
	...
	for _, cand := range candidateURLs {
		...
		// Fetch the master manifest once and classify it from the fetched
		// text (master = lists #EXT-X-STREAM-INF variants). Probing first and
		// re-fetching the full manifest afterwards used to cost a redundant
		// upstream round trip on every vidsrcme launch.
		...
		if !strings.Contains(manifestText, "#EXT-X-STREAM-INF") && !strings.Contains(manifestText, "#EXTINF") {
			lastErr = fmt.Errorf("playlist at %s has no variants or segments", redactQuery(finalURL))
			continue
		}
...
func (r *Resolver) decryptVidsrcmeStreamURLs(ctx context.Context, client *http.Client, apiRes vidsrcmeAPIResponse) ([]string, error) {
	...
	if len(res) == 0 {
		return nil, errors.New("wasm alloc returned no pointer")
	}
	ptr := uint32(res[0])
	...
	if len(decRes) == 0 {
		return nil, errors.New("wasm decrypt returned no output length")
	}
...
func (r *Resolver) getCompiledWasm(ctx context.Context, client *http.Client, rt wazero.Runtime, apiRes vidsrcmeAPIResponse) (wazero.CompiledModule, error) {
	...
	if apiRes.VS.Wasm != "" {
		...
		// The URL comes from the upstream data API — apply the same
		// SSRF guard every other upstream-derived URL in this package
		// goes through before the server fetches it.
		wu, err := url.Parse(apiRes.VS.WasmURL)
		if err != nil || (wu.Scheme != "https" && wu.Scheme != "http") || wu.Host == "" {
			return nil, fmt.Errorf("invalid wasm_url")
		}
		if r.blockedUpstreamHost(ctx, wu.Hostname()) {
			return nil, fmt.Errorf("wasm_url host %q blocked", strings.ToLower(wu.Host))
		}
