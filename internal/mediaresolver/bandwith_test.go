...
// about: vixsrc, all nine vidking source servers and all seven vidlove source
// servers. Unlike production Resolve() — which stops at the first server that
// yields a stream — this walks the full server lists so every upstream can be
...
//	BW_PROVIDERS=vixsrc,vidking,vidlove   (comma-separated subset)
//
