...
type resolutionRecord struct {
	...
	noRevalidate bool
	// tier is the provider's quality label for the picked variant
	// ("2160p", "auto") when reported — the dashboard's resolution column
	// for sources whose playlist itself carries no RESOLUTION.
	tier      string
	createdAt time.Time
}
...
func (r *Resolver) tryCachedResolution(req MediaRequest) (string, bool) {
	...
	// Dashboard resolution: the tier captured at resolve time and/or the
	// manifest text cover providers whose live playlists carry no
	// RESOLUTION attribute (vidking media playlists).
	masterText := ""
	if len(rec.manifest) > 0 {
		masterText = string(rec.manifest)
	}
	if masterText != "" || rec.tier != "" {
		r.noteStreamHeightToken(token, masterText, rec.tier)
	}
	if len(rec.manifest) > 0 {
