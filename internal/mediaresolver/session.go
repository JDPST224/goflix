...
type proxySession struct {
	...
	// createdAt anchors the stream age shown in the admin dashboard.
	createdAt time.Time
	// clientIP is the viewer address learned from the first proxy request
	// (an atomic.Value holding a string).
	clientIP atomic.Value
	// bytesServed counts bytes streamed to the player for the dashboard.
	bytesServed atomic.Int64
	// lastSeenAt is the unix-nano stamp of the most recent proxy request.
	lastSeenAt atomic.Int64
	// paused marks a stream held by an administrator from the dashboard.
	paused atomic.Bool
	// maxHeight is the pixel height of the top video variant in the
	// session's master playlist (2160 = 4K, 1080 = 1080p, …) for the
	// dashboard's resolution column. 0 until a master playlist is seen.
	maxHeight atomic.Int32
	// maxWidth is the pixel width of that same variant; the dashboard
