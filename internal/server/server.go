...
func New(d *Deps) http.Handler {
	// Normalize absent collaborators so handlers can rely on them being
	// non-nil: an empty auth store means open registration, an empty
	// catalog client means every TMDB-backed handler answers 503 instead
	// of panicking.
	if d.Auth == nil {
		d.Auth = NewAuthStore("", "")
	}
	...
	for _, provider := range []string{"cinesrc", "vixsrc", "vidking", "vidsrcme"} {
	...
	// End a proxy session at the viewer's request: the player retires its
	// previous session when launching a new one.
	mux.HandleFunc("/api/media/retire/", d.mediaRetireHandler)
	...
	// Admin stream monitoring + controls (live streams dashboard).
	mux.HandleFunc("/api/admin/streams", d.adminStreamsHandler)
	mux.HandleFunc("/api/admin/streams/stats", d.adminStreamStatsHandler)
	mux.HandleFunc("/api/admin/streams/", d.adminStreamActionHandler)
