...
func fetchSubtitleVTT(ctx context.Context, targetURL, referer string, allowedDomains []string, logProvider string) (string, int, string) {
	...
	// Decompress gzip payload if present. The decompressed output is bounded
	// as well: a 2 MiB gzip stream can legally expand to gigabytes, and the
	// result is buffered whole before conversion.
	if len(body) > 2 && body[0] == 0x1f && body[1] == 0x8b {
		unzipped, status, msg := decompressSubtitleBody(body, logProvider)
		if status != 0 {
			return "", status, msg
		}
		body = unzipped
...
// decompressSubtitleBody expands a gzip-compressed subtitle body, keeping
// both the compressed input and the decompressed output within the subtitle
// byte caps (status 0 means success).
func decompressSubtitleBody(body []byte, logProvider string) ([]byte, int, string) {
	gr, err := gzip.NewReader(bytes.NewReader(body))
	if err != nil {
		log.Printf("[Subtitles] %s gzip reader error: %v", logProvider, err)
		return nil, http.StatusBadGateway, "Subtitle decompression failed"
	}
	unzipped, err := io.ReadAll(io.LimitReader(gr, maxSubtitleBytes+1))
	gr.Close()
	if err != nil {
		log.Printf("[Subtitles] %s gzip read error: %v", logProvider, err)
		return nil, http.StatusBadGateway, "Subtitle decompression failed"
	}
	if len(unzipped) > maxSubtitleBytes {
		log.Printf("[Subtitles] %s gzip payload expanded past %d bytes", logProvider, maxSubtitleBytes)
		return nil, http.StatusBadGateway, "Subtitle file too large"
	}
	return unzipped, 0, ""
}
