...
func (d *Deps) mediaProxyHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := d.Resolver.Proxy(w, r, token); err != nil {
		...
		// Dashboard pause/block controls: answer the player without logging
		// so a held stream stalls silently until an admin resumes it.
		if errors.Is(err, mediaresolver.ErrStreamPaused) {
			writeError(w, http.StatusServiceUnavailable, "Stream paused by administrator")
			return
		}
		if errors.Is(err, mediaresolver.ErrStreamBlocked) {
			writeError(w, http.StatusForbidden, "Stream blocked by administrator")
			return
		}
...
// mediaRetireHandler lets the player explicitly end its previous proxy
// session when it starts a new one (title, episode or server switch), so the
// abandoned session stops counting as an active stream immediately instead
// of lingering through its idle window.
func (d *Deps) mediaRetireHandler(w http.ResponseWriter, r *http.Request) {
	if !corsGate(w, r, "POST", false) {
		return
	}
	token := strings.Trim(strings.TrimPrefix(r.URL.Path, "/api/media/retire/"), "/")
	token = strings.TrimSuffix(token, ".m3u8")
	if token == "" {
		writeError(w, http.StatusBadRequest, "Invalid proxy token")
		return
	}
	d.Resolver.RetireStream(token)
	writeJSON(w, http.StatusOK, map[string]any{"success": true})
}
