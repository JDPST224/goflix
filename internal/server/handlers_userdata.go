...
// union. Each account owns an isolated state (v2 store); devices signed into
// the same account share it and different accounts never see each other's
// data.
...
func mediaRefKey(raw json.RawMessage) string {
	...
	case string:
		return t + "-" + id
	case float64:
		// FormatFloat with precision -1 renders integral floats without a
		// decimal point ("550") and keeps fractional IDs intact ("550.5").
		// The old TrimSuffix chain stripped real trailing digits ("550" →
		// "55"), colliding distinct titles under one media key.
