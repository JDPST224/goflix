...
// allowedWrapPaths lists the download endpoints that actually emit WebVTT.
var allowedWrapPaths = []string{
	"/api/subtitles/vidsrcme/download",
	"/api/subtitles/cinesrc/download",
	"/api/subtitles/opensubtitles/download",
}

// validLocalSubtitleWrapSrc restricts the wrap endpoint's src parameter to
// the download endpoints in allowedWrapPaths, so a wrap playlist cannot
// chain into another wrap or fetch a JSON search endpoint.
func validLocalSubtitleWrapSrc(p string) bool {
	...
	return slices.Contains(allowedWrapPaths, u.Path)
}
...
// resolver: it runs during Resolve() so VidKing sessions get their
