...
func (c *Client) tvSeason(id, season string) ([]byte, bool) {
	key := id + ":" + season
	// Fast path: a fresh cached entry answers without touching the network.
	c.seasonMu.Lock()
	...
	// Fetch with the lock RELEASED: holding it across the HTTP round trip
	// serialized every concurrent episode lookup behind one 15s-timeout
	// request. Concurrent misses of the same season may fetch in parallel;
	// the re-check below keeps the last successful write.
