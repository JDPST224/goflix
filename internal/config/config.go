...
func Load(path string) (Config, error) {
	...
	for scanner.Scan() {
		...
		// Quote-cleaning up front so numeric/bool keys honor the same
		// KEY="value" convention the string keys do — strconv would reject
		// a quoted value and silently keep the default.
		val := cleanConfigValue(strings.TrimSpace(parts[1]))
		switch key {
		...
		case "BLOCKLIST_FILE":
			cfg.Resolver.BlockListPath = cleanConfigValue(val)
		case "BROWSER_EXECUTABLE":
			cfg.Resolver.BrowserExecutable = cleanConfigValue(val)
		case "CACHE_MAX_MB":
			if v, err := strconv.Atoi(val); err == nil && v > 0 {
...
func applyEnvOverrides(cfg *Config) {
	...
	if v := os.Getenv("BLOCKLIST_FILE"); strings.TrimSpace(v) != "" {
		cfg.Resolver.BlockListPath = cleanConfigValue(v)
	}
