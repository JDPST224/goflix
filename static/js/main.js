import { PLACEHOLDER_POSTER, LIST_CHECK_SVG, LIST_PLUS_SVG,
         PLACEHOLDER_AVATAR, PLACEHOLDER_STILL, escapeHtml, mediaKey,
         formatPlayerTime, isHlsServer } from './utils.js';
import { getMyList, isInMyList, toggleMyList, getProgress,
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const playerSeekPreview   = document.getElementById('player-seek-preview');
    const playerSettingsPicker  = document.getElementById('player-settings-picker');
    const playerSettingsTrigger = document.getElementById('player-settings-trigger');
    const playerSettingsMenu    = document.getElementById('player-settings-menu');
    const playerSpeedCurrent    = document.getElementById('player-speed-current');
    ...
    // Source URL of the session currently loaded in the player. A new launch
    ...
    let externalSubtitleTracks = []; // Subtitles fetched from OpenSubtitles via the backend
    ...
    function closeSearch() {
        ...
        searchResultsArea.setAttribute('aria-busy', 'false');
    }
    ...
    searchInput.addEventListener('input', () => {
        ...
        searchResultsArea.setAttribute('aria-busy', 'true');
        searchResultsGrid.innerHTML = '';
    ...
    async function fetchExternalSubtitles(movie, season, episode, provider) {
        ...
        const supportedProviders = ['cinesrc', 'vidking', 'vidsrcme'];
    ...
    async function launchPlayer(movie, season, episode) {
        ...
        // Retire the previous playback session immediately: a title, episode
        // or server switch abandons the old proxy session, and without this
        // the dashboard counts the same viewer as two devices until the
        // abandoned session's idle window elapses.
        retireActiveSession();
        ...
        const provider = server === 'cinesrc' ? 'cinesrc' : (server === 'vidking' ? 'vidking' : (server === 'vidsrcme' ? 'vidsrcme' : 'vixsrc'));
        ...
        return new Promise((resolve, reject) => {
            ...
            if (window.Hls && Hls.isSupported()) {
                ...
                vixHlsInstance = new Hls({
                    ...
                    autoStartLoad: true,
                    startPosition: pendingResumePosition > 5 ? pendingResumePosition : -1,
    ...
    function closeAllPickers(exceptPicker = null) {
        [[playerServerPicker, playerServerTrigger],
         ...
         [playerSeasonPicker, playerSeasonTrigger],
         [playerSettingsPicker, playerSettingsTrigger]].forEach(([picker, trigger]) => {
    ...
    // ─── Skip buttons ─────────────────────────────────────────────────────
    // Relative seek that clamps to the media bounds. Mirrors the double-tap
    // zones and the arrow-key shortcuts so all three paths behave alike.
    function skipBy(seconds) {
        if (!isHlsServer(activePlayerServer) || !playerReady) return;
        if (!Number.isFinite(vixPlayer.currentTime)) return;
        const limit = Number.isFinite(vixPlayer.duration) ? vixPlayer.duration : Infinity;
        vixPlayer.currentTime = Math.max(0, Math.min(limit, vixPlayer.currentTime + seconds));
        showControls();
    }

    playerSkipBack?.addEventListener('click', () => skipBy(-10));
    playerSkipForward?.addEventListener('click', () => skipBy(10));

    // ─── Playback speed ───────────────────────────────────────────────────
    const PLAYBACK_SPEEDS = [
        { rate: 0.5,  label: '0.5×', note: 'Quarter pace' },
        { rate: 0.75, label: '0.75×', note: 'Slower' },
        { rate: 1,    label: 'Normal', note: 'Default' },
        { rate: 1.25, label: '1.25×', note: 'Slightly faster' },
        { rate: 1.5,  label: '1.5×', note: 'Faster' },
        { rate: 2,    label: '2×', note: 'Double pace' },
    ];

    function setPlaybackSpeed(rate, announce = false) {
        if (!vixPlayer) return;
        const entry = PLAYBACK_SPEEDS.find(s => s.rate === rate) || PLAYBACK_SPEEDS[2];
        try { vixPlayer.playbackRate = entry.rate; } catch (_) { return; }
        playerSettingsMenu?.querySelectorAll('[data-rate]').forEach(btn => {
            btn.setAttribute('aria-selected', String(Number(btn.dataset.rate) === entry.rate));
        });
        if (playerSpeedCurrent) playerSpeedCurrent.textContent = `Speed ${entry.label}`;
        if (playerSettingsTrigger) {
            const text = entry.rate === 1 ? 'Normal' : entry.label;
            playerSettingsTrigger.title = `Playback speed: ${text}`;
            playerSettingsTrigger.setAttribute('aria-label', `Playback settings, speed ${text}`);
            // Off-default speeds get a visible marker so the state isn't hidden
            // behind a menu the user has to open to check.
            playerSettingsTrigger.classList.toggle('is-modified', entry.rate !== 1);
        }
        if (announce) showToast(entry.rate === 1 ? 'Normal speed' : `Speed ${entry.label}`);
    }

    // Step through the preset ladder rather than free-forming the rate, so the
    // keyboard and the menu can never disagree about the current value.
    function nudgePlaybackSpeed(direction) {
        const current = PLAYBACK_SPEEDS.findIndex(s => s.rate === vixPlayer.playbackRate);
        const from = current === -1 ? 2 : current;
        const next = Math.max(0, Math.min(PLAYBACK_SPEEDS.length - 1, from + direction));
        setPlaybackSpeed(PLAYBACK_SPEEDS[next].rate, true);
        showControls();
    }

    if (playerSettingsMenu) {
        const heading = document.createElement('div');
        heading.className = 'player-menu-heading';
        heading.textContent = 'Playback speed';
        playerSettingsMenu.appendChild(heading);
        PLAYBACK_SPEEDS.forEach(({ rate, label, note }) => {
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.setAttribute('role', 'option');
            btn.dataset.rate = String(rate);
            btn.setAttribute('aria-selected', String(rate === 1));
            btn.innerHTML = `<span><strong></strong><small></small></span>
                <svg class="track-option-check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg>`;
            btn.querySelector('strong').textContent = label;
            btn.querySelector('small').textContent = note;
            btn.addEventListener('click', () => {
                setPlaybackSpeed(rate, true);
                playerSettingsPicker?.classList.remove('open');
                playerSettingsTrigger?.setAttribute('aria-expanded', 'false');
                showControls();
            });
            playerSettingsMenu.appendChild(btn);
        });
    }

    playerSettingsTrigger?.addEventListener('click', e => {
        e.stopPropagation();
        toggleTrackPicker(playerSettingsPicker, playerSettingsTrigger);
    });

    // ─── Seek hover preview ───────────────────────────────────────────────
    // Shows the timestamp under the pointer while hovering the seek bar, so a
    // scrub target is known before committing to it.
    function updateSeekPreview(clientX) {
        if (!playerSeekPreview || !playerProgressTrack || !playerProgress) return;
        const duration = Number.isFinite(vixPlayer.duration) ? vixPlayer.duration : 0;
        if (!duration || !isHlsServer(activePlayerServer)) {
            playerProgressTrack.classList.remove('preview-on');
            return;
        }
        const rect = playerProgress.getBoundingClientRect();
        if (!rect.width) return;
        const ratio = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
        playerSeekPreview.textContent = formatPlayerTime(ratio * duration);
        // Clamp so the bubble never hangs off either end of the track.
        const half = playerSeekPreview.offsetWidth / 2;
        const x = Math.max(half, Math.min(rect.width - half, ratio * rect.width));
        playerSeekPreview.style.left = `${x}px`;
        playerProgressTrack.classList.add('preview-on');
    }

    playerProgressTrack?.addEventListener('mousemove', e => updateSeekPreview(e.clientX));
    playerProgressTrack?.addEventListener('mouseleave', () => playerProgressTrack.classList.remove('preview-on'));
    playerProgress?.addEventListener('input', () => {
        const rect = playerProgress.getBoundingClientRect();
        updateSeekPreview(rect.left + (Number(playerProgress.value) / 100) * rect.width);
    });
    ...
    // retireActiveSession tells the server to end the proxy session of the
    // source currently loaded in the player. Fire-and-forget: the dashboard's
    // stream list should drop the entry at once, and failure is irrelevant
    // (the session still expires normally).
    function retireActiveSession() {
        if (!activeSourceUrl) return;
        const match = String(activeSourceUrl).match(/\/api\/media\/proxy\/([A-Za-z0-9_-]+)\.m3u8/);
        activeSourceUrl = null;
        if (match) fetch(`/api/media/retire/${match[1]}`, { method: 'POST' }).catch(() => {});
    }
    ...
    function closePlayerModal() {
        ...
        // End the playback session with it: a closed player is not an active
        // stream, so retire instead of idling out over the next two minutes.
        retireActiveSession();
    ...
    function anyPickerOpen() {
        return [playerServerPicker, playerAudioPicker, playerSubtitlePicker, playerSeasonPicker, playerSettingsPicker]
    ...
    vixPlayer.addEventListener('timeupdate', () => {
        ...
        if (currentPlayerMovie && mediaLive && (vixPlayer.currentTime - lastSavedPlaybackSecond >= 5 || vixPlayer.currentTime < lastSavedPlaybackSecond - 2)) {
            ...
            // after rewinding silently resumed past what the user skipped.
            saveProgress(currentPlayerMovie, currentPlayerSeason, currentPlayerEpisode, vixPlayer.currentTime, vixPlayer.duration);
            lastSavedPlaybackSecond = vixPlayer.currentTime;
        }
    ...
    const applyPendingResume = () => {
        if (vixPlayer.__resumeApplied) return true;
        if (pendingResumePosition <= 5) {
    ...
    document.addEventListener('keydown', e => {
        ...
        if (playerOpen) {
            ...
            if (e.key === 'j' || e.key === 'J') { e.preventDefault(); skipBy(-10); return; }
            if (e.key === 'l' || e.key === 'L') { e.preventDefault(); skipBy(10); return; }
            if (e.key === '<' || e.key === ',') { e.preventDefault(); nudgePlaybackSpeed(-1); return; }
            if (e.key === '>' || e.key === '.') { e.preventDefault(); nudgePlaybackSpeed(1); return; }
            // Number keys jump to that tenth of the runtime, YouTube-style.
            if (/^[0-9]$/.test(e.key) && Number.isFinite(vixPlayer.duration) && vixPlayer.duration > 0) {
                e.preventDefault();
                vixPlayer.currentTime = (Number(e.key) / 10) * vixPlayer.duration;
                showControls();
                return;
            }
