...
export function clearLocalUserData() {
    ['goflix_mylist', 'goflix_progress', 'goflix_cw', 'goflix_removed', 'goflix_avprefs', USER_KEY].forEach(k => {
...
async function syncNow() {
    // Never snapshot local data before the initial binding decision: a
    // mutation during the /api/auth/status round trip could otherwise
    // upload the previous account's snapshot under the new account.
...
// initPromise resolves once the binding decision (keep / wipe / anonymous)
// is made; syncNow awaits it before its first upload.
let initPromise = (async function initialSync() {
    let user = '';
    try {
